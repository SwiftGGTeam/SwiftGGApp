...
public final class ImageViewer: UIViewController, UIScrollViewDelegate, UIViewControllerTransitioningDelegate {
    ...
    private func configureImageView() {
        ...
        parentViewFrameInOurCoordinateSystem = CGRectIntegral(applicationWindow!.convertRect(view.bounds, fromView: view))
        
        imageView.frame = parentViewFrameInOurCoordinateSystem
        imageView.contentMode = .ScaleAspectFit
        view.addSubview(imageView)
        imageView.image = screenshotFromView(view)
    }
    ...
    func rotate() {
        ...
        let aspectFitSize = aspectFitContentSize(forBoundingSize: rotationAdjustedBounds().size, contentSize: view.frame.size)
    ...
    func closeAnimation(duration: NSTimeInterval, completion: ((Bool) -> Void)?) {
        ...
        UIView.animateWithDuration(duration, animations: {
            ...
            self.imageView.frame = CGRectIntegral(self.applicationWindow!.convertRect(self.view.bounds, fromView: self.view))
    ...
    override public func observeValueForKeyPath(keyPath: String?, ofObject object: AnyObject?, change: [String: AnyObject]?, context: UnsafeMutablePointer<Void>) {
        ...
        if (dynamicTransparencyActive == true && keyPath == "contentOffset") {
            ...
            view.alpha = 1 - fabs(scrollView.contentOffset.y / distanceToEdge)
